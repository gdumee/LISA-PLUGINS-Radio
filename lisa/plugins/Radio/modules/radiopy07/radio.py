...
if os.path.exists(os.path.dirname(os.path.abspath(__file__))+ '/radiopy07') :
    import radiopy07.radiopy
else :
    import radiopy
...
def parse_arguments():
    ...
    parser.add_argument("-m", "--mute", dest="mute", action="store_true", default=False,                                    #new
                    help="mute output")
    ...
    #logging.critical("Start {}".format(__file__))
...
def main():
    ...
    if args.random:
        ...
        ret = player.play(args._station_name, **player_options)
    
    print 'radiopy return :',ret   # 2)but print yes, because you can get it in PIPE of subprocess.
    return ret  #1)return doesn't work with subprocess because end of subprocess return 0 because subprocess is correctly finish. You know ?.
